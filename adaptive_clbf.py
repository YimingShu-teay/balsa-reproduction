...
class AdaptiveClbf(object):
    def __init__(self,odim=2, use_service = True, ):
        self.xdim = 4
        self.udim = 2
        self.odim = odim#一个和theta相关的量，不太知道具体是什么。
    ...
    def get_control(self,z,z_ref,z_ref_dot,dt,obs=None,train=False,use_model=False,add_data=True,check_model=True,use_qp=True):
        ...
        mDelta = np.zeros((self.xdim//2,1),dtype=np.float32)#均值
        sigDelta = np.zeros((self.xdim//2,1),dtype=np.float32)#方差
        rho = np.zeros((self.xdim//2,1),dtype=np.float32)#这是什么
        ...
        #增加数据
        if add_data:
                req = AddData_Model(self.z.flatten(), self.z_prev.flatten(),mu_model.flatten(),self.obs_prev.flatten(),dt)
                self.model.add_data(req)

        self.z_dot = (self.z[2:-1,:]-self.z_prev[2:-1,:])/dt - mu_model
        
        #所以check_model和use_model这两个有什么区别呢
        if check_model and self.model_trained:
            ...
            if self.use_service:
                ...
                req = Predict_Runtime(self.z_prev.flatten(),self.obs_prev.flatten())
            ...
            if predict_service_success:
                ...
                #预测误差，也就是计算出的delta和高斯预测出的delta的差值。
        ...
        if use_model and self.model_trained:
            ...
            sigDelta = np.ones((self.xdim//2,1))#为什么这样设呢，稍稍有点奇怪
