...
params={}
params["vehicle_length"] = 0.25#车长
params["steering_limit"] = 0.75#前轮转向角限制
params["max_accel"] = 1.0#最大加速度
params["min_accel"] = -1.0#最小加速度
params["kp_z"] = 1.0#比例增益
params["kd_z"] = 1.0#微分增益
params["clf_epsilon"] = 100.0 #应该是CLF的gamma


params["qp_u_cost"] = 100.0
params["qp_u_prev_cost"] = 1.0
params["qp_p1_cost"] = 1.0#clf松弛变量前面的系数
params["qp_p2_cost"] = 1.0e12#cbf松弛变量前面的系数
params["qp_max_var"] = 1.5
params["qp_verbose"] = False
params["max_velocity"] = 2.0#最大速度限制
params["min_velocity"] = 0.5#最小速度限制
params["barrier_vel_gamma"] = 10.0#应该是cbf的gamma
params["use_barrier_vel"] = True
params["use_barrier_pointcloud"] = True
params["barrier_radius"] = 1.0#障碍物在barrier function中的波及范围
...
dt = 0.1
N = int(round(T/dt))
t = np.linspace(0,T-2*dt,N-1)#399
xdim=4
udim=2
...
speed = 1.0
freq = 1.0/10
x_d = np.stack((t * speed, width * np.sin(2 * np.pi * t * freq), np.zeros(N-1), np.zeros(N-1)))#(4,399)
x_d[2,:-1] = np.arctan2(np.diff(x_d[1,:]),np.diff(x_d[0,:]))#x_d[0]--x,  x_d[1]--y,  x_d[2]--theta,  x_d[3]--v
...
for i in range(N-2):
	...
	prediction_error[i] = adaptive_clbf.predict_error #check_model后拿到的predict_error
	prediction_error_true[i] = adaptive_clbf.true_predict_error #use_model后拿到的true_predict_error
